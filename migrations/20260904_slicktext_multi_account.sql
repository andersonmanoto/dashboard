...
-- Conta original, a mesma já usada via SLICKTEXT_API_KEY/SLICKTEXT_BRAND_ID
-- no .env. O código continua lendo a api_key dessa conta a partir dessas
-- variáveis (sem sufixo) quando slicktext_accounts.brand_id = SLICKTEXT_BRAND_ID
-- -- o brand_id que já está no .env atual é '34641'.
...
-- Backfill: todo produto que já tinha list_id mapeado migra para a conta original.
...
-- Produtos que passam a usar a conta Velupet (sobrescreve o mapeamento da
-- conta original feito pelo backfill acima, para estes 5 produtos).
insert into public.slicktext_product_lists (product_id, slick_account_id, approved_list_id, abandoned_list_id)
...
on conflict (product_id) do update
    set slick_account_id = excluded.slick_account_id,
